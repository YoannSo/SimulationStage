...
void display()
{
    ...
    if (!bPause)
    {
        ...
        if(meshRenderer)
            meshRenderer->setVertexBuffer(psystem->getCurrentReadBufferTriangle(), psystem->getNumTriangles());
...
void key(unsigned char key, int /*x*/, int /*y*/)
{
    ...
    case 13:
        ...
        if(meshRenderer)
            meshRenderer->setVertexBuffer(psystem->getCurrentReadBufferTriangle(),3);
...
void initParams()
{
    ...
    else
    {
        ...
        params->AddParam(new Param<int>("Nb Cycles", maxCycles, 100, 500, 1, &maxCycles));
        params->AddParam(new Param<float>("Pump Force", pumpForce, 1.0f, 0.f, 0.1f, &pumpForce));
        params->AddParam(new Param<float>("Inclinaison", inclinaison, 0.0001f, -0.0001f, 0.00001f, &inclinaison));
        params->AddParam(new Param<float>("cube size pronfondeur", cubeSizeZ, -0.9f, 1.f, 0.001f, &cubeSizeZ));
        params->AddParam(new Param<float>("cube size largeur", cubeSizeX, 1.0f, 2.f, 0.1f, &cubeSizeX));
