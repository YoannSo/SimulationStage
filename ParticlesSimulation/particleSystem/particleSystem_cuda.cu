...
extern "C"
{
    ...
    void launchFlowForce(float* newVel,
        ...
        getFlowForce << <numBlocks, numThreads >> > ((float4*)newVel,
            (float4*)sortedPos,
            (float4*)sortedVel,
            gridParticleIndex,
            cellStart,
            cellEnd,
            numParticles, inclinaison, pumpForce);
    ...
    {
        thrust::sort_by_key(thrust::device_ptr<uint>(dGridTriangleHash),
            thrust::device_ptr<uint>(dGridTriangleHash + numTriangles),
            thrust::device_ptr<uint>(dGridTriangleIndex));
    }
}   // extern "C"
