...
const uint width = 720, height = 460;
...
MeshRenderer::DisplayMode meshDisplayMode = MeshRenderer::PARTICLE_SPHERES;
CopperBallsRenderer::DisplayMode copperBallsDisplay = CopperBallsRenderer::PARTICLE_SPHERES;
int mode = 0;
...
float meshTranslateX = 0.f;
float meshTranslateY = 0.f;
float meshTranslateZ = 0.f;
...
CopperBallsRenderer* copperBallsRenderer = 0;
float modelView[16];
...
{
    psystem = new ParticleSystem(numParticles, gridSize, bUseOpenGL,MESH_NAME,pourcentageCopperBall);
    ...
    if (bUseOpenGL)
    {
        ...
        copperBallsRenderer = new CopperBallsRenderer;
        copperBallsRenderer->setParticleRadius(0.005);
        copperBallsRenderer->setColorBuffer(psystem->getColorBuffer());
...
void display()
{
    ...
    if (!bPause && launchSimulation)
    {
        ...
        psystem->setMaxCycles(maxCycles);
        psystem->setPumpForce(pumpForce);
        psystem->setInclinaison(inclinaison);
        psystem->setCubeSizeZ(cubeSizeZ);
        psystem->setCubeSizeX(cubeSizeX);
        ...
        if (renderer)
        {
            ...
            copperBallsRenderer->setPositions(psystem->getCopperBallsVertexBuffer(), psystem->getNumCopperBalls());
...
>>>>>>> f77b5d5a58d4ac1b8aa7286792f06f1c8c2a9e60
    ...
    float3 t1 = psystem->getP0();
    float3 t2 = psystem->getP1();
    float3 t3 = psystem->getP2();
    float3 t4 = psystem->getP3();
...
>>>>>>> f77b5d5a58d4ac1b8aa7286792f06f1c8c2a9e60
        ...
        for (float i = 0.f; i < GRID_SIZE; i++) {
            glPushMatrix(); 
            glTranslatef(i*-cellSize.x, 0.f, 0.f);
            glutWireCube(cellSize.x);//sets scale of wire cube
            glPopMatrix();
            for (float j = 0.f; j < GRID_SIZE; j++) {
                glPushMatrix();
                glTranslatef(i * -cellSize.x, j * -cellSize.y, 0.f);
                glutWireCube(cellSize.y);//sets scale of wire cube
                glPopMatrix();
                for (float w = 0.f; w < GRID_SIZE; w++) {
                    glPushMatrix();
                    glTranslatef(i * -cellSize.x, j * -cellSize.y, w * -cellSize.z);
                    glutWireCube(cellSize.z);//sets scale of wire cube
                    glPopMatrix();
                }
            }
        }
...
>>>>>>> f77b5d5a58d4ac1b8aa7286792f06f1c8c2a9e60
    ...
    if (copperBallsRenderer && displayEnabled && displayCopperBalls)
    {
        copperBallsRenderer->display(copperBallsDisplay);
    }
    if (displaySliders)
    {
...
void reshape(int w, int h)
{
    ...
    glMatrixMode(GL_MODELVIEW);
    glViewport(0, 0, w, h);
    glm::mat4 projectionMatrix= glm::perspective(glm::radians(60.f), (float)w / (float)h, 0.1f, 100.f);
    meshRenderer->setProjectionMatrix(projectionMatrix);
    if (renderer)
    {
    ...
    if (meshRenderer)
    {
        meshRenderer->setWindowSize(w, h);
        meshRenderer->setFOV(60.0);
    }
    if (copperBallsRenderer)
    {
        copperBallsRenderer->setWindowSize(w, h);
        copperBallsRenderer->setFOV(60.0);
    }
}
...
void motion(int x, int y)
{
    ...
    case M_MOVE:
    {
        ...
        triangles[4] = p1;
        triangles[5] = p3;
        psystem->addAllTriangles(6, triangles);
...
void key(unsigned char key, int /*x*/, int /*y*/)
{
    ...
    case 'm':
        ...
        break;
    case 'c':
        displayCopperBalls = !displayCopperBalls;
        break;
    ...
    case 'd':
        //psystem->dumpGrid();
        meshRenderer->changeRenderState();
        break;
...
>>>>>>> f77b5d5a58d4ac1b8aa7286792f06f1c8c2a9e60
    ...
    case 'a':
        renderer->setColorBuffer(psystem1->getColorBuffer());
...
void initParams()
{
    ...
    else
    {
        ...
        params->AddParam(new Param<float>("Mesh Translate X", meshTranslateX, 3.f, -3.f, 0.2f, &meshTranslateX));
        params->AddParam(new Param<float>("Mesh Translate Y", meshTranslateY, 3.f, -3.f, 0.2f, &meshTranslateY));
        params->AddParam(new Param<float>("Mesh Translate Z", meshTranslateZ, 3.f, -3.f, 0.2f, &meshTranslateZ));
...
void initMenus()
{
    ...
    glutAddMenuEntry("1 particle colored [a]", 'a');
    glutAddMenuEntry("All particles colored [b]", 'b');
